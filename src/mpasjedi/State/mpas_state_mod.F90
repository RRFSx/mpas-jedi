...
subroutine add_incr(self, increment)
   ...
   logical, pointer :: config_da_update_2mTQ => null()
   ...
   if (self%geom%nCells==increment%geom%nCells .and. self%geom%nVertLevels==increment%geom%nVertLevels) then
      ...
      call mpas_pool_get_config(self%geom%domain%blocklist%configs, 'config_da_update_2mTQ', config_da_update_2mTQ)
      if (associated(config_da_update_2mTQ) .and. config_da_update_2mTQ) then
